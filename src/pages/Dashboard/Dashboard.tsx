...
const LiveAccount: React.FC<{ appUser: AppUserModel; getUser: Function }> = ({
  ...
  useEffect(() => {
    ...
    const hasMT5Accounts = appUser.UserDtos && appUser.UserDtos.length > 0;
    ...
    getUserAccount();
  }, [appUser, accountStatus, showSuccessMessage]);
  ...
  return (
    <>
      <Row>
        <Space>
          {isAdmin && appUser.UserDtos && appUser.UserDtos.length > 0 && (
            <Button
              type={'primary'}
              onClick={async () => {
                try {
